...
**Название:** AI-Enhanced Rap Lyrics Analysis & Dataset Creation  ### 3. `multi_model_analyzer.py` - AI анализатор
**Назначение:** Анализ текстов песен через локальные модели (Ollama) или DeepSeek API

**Ключевые методы:**
```python
class MultiModelAnalyzer:
    def analyze_song()            # Анализ с fallback между провайдерами
    def batch_analyze_from_db()   # Массовый анализ из БД
    def _save_analysis_to_db()    # Сохранение результатов
    def get_stats()               # Статистика использования
```

**Технические особенности:**
- Приоритет: Ollama (бесплатно, локально)
- Fallback: DeepSeek API (дешево)
- Автоматическое переключение при ошибках
- Кэширование результатов (в планах)ие высококачественного датасета с AI-анализом текстов рэп-песен для обучения ML моделей генерации музыки  
...
- **DeepSeek API:** Гибкие лимиты по токенам
- **Автоматический retry** и fallback между провайдерами
...
### 5. Запуск AI анализа:
```bash
python multi_model_analyzer.py  # Запуск анализа
```
...
*"Я создал end-to-end ML pipeline для анализа 44,000+ рэп-песен с помощью локальных LLM (Ollama) и DeepSeek API. Система автоматически извлекает 20+ структурированных признаков из неструктурированного текста - от жанра до метрик аутентичности. Результат: высококачественный датасет для обучения моделей генерации 'живой' музыки с возможностью бесплатного и неограниченного анализа."*
