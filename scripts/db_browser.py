...
class DatabaseBrowser:
    ...
    async def show_stats(self):
        ...
        async with self.db.get_connection() as conn:
            ...
            for analyzer in analyzer_stats:
                print(f"  {analyzer['analyzer_type']}: {analyzer['analysis_count']} анализов")
    ...
    async def show_track(self, track_id: int):
        ...
        async with self.db.get_connection() as conn:
            ...
            for analysis in analyses:
                ...
                print(f"   Сложность: {analysis['complexity_score'] or 'Не определена'}")
                ...
                if analysis['analysis_data']:
                    try:
                        ...
                        print(f"   Энергия: {data.get('energy_level', 'Не определена')}")
                        print(f"   Качество: {data.get('overall_quality', 'Не определено')}")
                    except (json.JSONDecodeError, AttributeError):
                        print("   Дополнительные данные: ошибка парсинга")
                print("-" * 20)
    
    async def list_tracks(self, artist: Optional[str] = None, limit: int = 20):
        """Список треков"""
        async with self.db.get_connection() as conn:
            if artist:
                tracks = await conn.fetch("""
            ...
            print("=" * 80)
            for track in tracks:
                release_year = track['release_date'].year if track['release_date'] else '????'
                print(f"ID: {track['id']:6} | {track['artist'][:20]:20} | {track['title'][:40]:40} | {release_year}")
    ...
    async def recent_analyses(self, limit: int = 10):
        ...
        async with self.db.get_connection() as conn:
            ...
            for analysis in analyses:
                print(f"🤖 {analysis['analyzer_type']:15} | {analysis['artist'][:20]:20} - {analysis['title'][:30]:30}")
                print(f"   Настроение: {analysis['sentiment'] or 'Не определено':15} | Создан: {analysis['created_at']}")
                print("-" * 80)

async def main():
    """Главная функция"""
    parser = argparse.ArgumentParser(description="Database Browser для PostgreSQL")
    parser.add_argument("command", choices=['stats', 'search', 'track', 'list', 'recent'], 
