...
from src.interfaces.database_interface import SQLiteManager, DatabaseConfig, create_database_manager
...
class Application:
    ...
    def _setup_database(self) -> None:
        ...
        try:
            ...
            self.database = SQLiteManager(db_config)
            
            # Test connection
            stats = self.database.get_analysis_stats()
            self.logger.info(f"Database connected: {stats['total_songs']} songs, "
                           f"{stats['songs_with_lyrics']} with lyrics")
    ...
    def get_database(self) -> SQLiteManager:
