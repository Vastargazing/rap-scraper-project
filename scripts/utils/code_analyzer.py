...
Анализ кода для выявления кандидатов на рефакторинг и дублирования

НАЗНАЧЕНИЕ:
- Поиск больших файлов и потенциальных дубликатов функций
- Анализ импортов и зависимостей для оптимизации
- Подготовка отчетов по возможным местам рефакторинга
...
ЗАВИСИМОСТИ:
- Python 3.8+
- Стандартная библиотека (ast, pathlib, json)

РЕЗУЛЬТАТ:
- JSON-отчет с метриками и предложениями для рефакторинга
- Консольный вывод сводки
...
class CodeAnalyzer:
    ...
    def analyze_function_names(self):
            ...
            try:
                ...
                for func_name in functions:
                    ...
                    self.results["function_names"].append(
                        {"function": func_name, "file": relative_path}
                    )
        ...
        for func_name, count in function_counter.items():
            if count > 1:
                files_with_func = [
                    item["file"]
                    for item in self.results["function_names"]
                    if item["function"] == func_name
                ]
                self.results["potential_duplicates"].append(
                    {"function": func_name, "count": count, "files": files_with_func}
                )
    ...
    def _categorize_file(self, file_path: str) -> str:
        ...
        if "analyzer" in file_path:
            return "analyzer"
        if "scraper" in file_path:
            return "scraper"
        if "enhancer" in file_path:
            return "enhancer"
        if "cli" in file_path or file_path.endswith("_cli.py"):
            return "cli"
        if "test" in file_path:
            return "test"
        if "util" in file_path:
            return "utility"
        return "other"
    ...
    def generate_report(self) -> dict:
        ...
        self.results["potential_duplicates"].sort(
        ...
        self.results["summary"] = {
            ...
            "refactoring_opportunities_count": len(
                self.results["refactoring_opportunities"]
            ),
        }
    ...
    def print_summary(self):
        ...
        print(
            f"   Potential duplicates: {summary.get('potential_duplicates_count', 0)}"
        )
        print(
            f"   Total opportunities: {summary.get('refactoring_opportunities_count', 0)}"
        )
        ...
        for i, file_info in enumerate(self.results["file_sizes"][:5]):
            print(
                f"   {i + 1}. {file_info['file']}: {file_info['lines']} lines ({file_info['category']})"
            )

        # High priority refactoring opportunities
        high_priority = [
            opp
            for opp in self.results["refactoring_opportunities"]
