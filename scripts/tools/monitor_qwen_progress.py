...
def monitor_qwen_progress():
    ...
    try:
        ...
        # Вывод статистики
        qwen_progress_percent = (
            (qwen_analyzed / total_records) * 100 if total_records > 0 else 0
        )
        all_progress_percent = (
            (all_analyzed / total_records) * 100 if total_records > 0 else 0
        )
        remaining_total = total_records - all_analyzed
        remaining_qwen = total_records - qwen_analyzed

        print("📈 Общий прогресс анализа:")
        print(f"  📊 Всего записей в базе: {total_records:,}")
        print(
            f"  ✅ Проанализировано всеми моделями: {all_analyzed:,} ({all_progress_percent:.1f}%)"
        )
        print(
            f"  🤖 Проанализировано Qwen: {qwen_analyzed:,} ({qwen_progress_percent:.2f}%)"
        )
        ...
        if time_stats[0] and qwen_analyzed > 0:  # Если есть Qwen анализы
            ...
            if duration.total_seconds() > 0:
                ...
                estimated_remaining_hours = (
                    remaining_qwen / rate if rate > 0 else float("inf")
                )

                print("\n⏱️  Qwen временная статистика:")
                print(
                    f"  🚀 Первый анализ: {first_analysis.strftime('%Y-%m-%d %H:%M:%S')}"
                )
                print(
                    f"  🏁 Последний анализ: {last_analysis.strftime('%Y-%m-%d %H:%M:%S')}"
                )
                ...
                if estimated_remaining_hours < float("inf"):
                    if estimated_remaining_hours < 24:
                        print(
                            f"  ⏰ Оставшееся время: {estimated_remaining_hours:.1f} часов"
                        )
                    else:
                        print(
                            f"  ⏰ Оставшееся время: {estimated_remaining_hours / 24:.1f} дней"
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=int, default=60, help="Интервал обновления в секундах"
    )
