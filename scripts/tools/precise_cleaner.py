...
def precision_clean_lyrics(lyrics, song_title=None):
    ...
    for pattern in patterns:
        ...
        if match:
            clean_content = match.group(1).strip() if pattern != patterns[4] else match.group(2).strip()
    ...
    if clean_content == lyrics and len(lyrics) > 300:
        ...
        for i, line in enumerate(lines):
            ...
            if (repeat_count < 3 and
                not re.search(r'\b(?:Contributors?|version|album|single|track|song|features?|establishes|describes|pictures|Many|This|The|\(.*?;.*?\))\b', line, re.IGNORECASE)):
                clean_content = '\n'.join(lines[i:])
                break
