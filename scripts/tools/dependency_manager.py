...
class DependencySecurityManager:
    ...
    def check_vulnerabilities(self) -> list[Vulnerability]:
        ...
        try:
            ...
            if shutil.which("safety") is None:
                print(
                    "safety не найден в PATH – пропуск проверки уязвимостей. Установите его через `pip install safety` для полного аудита."
                )
                return []
            ...
            print(
                "safety не найден в системе (FileNotFoundError). Установите его: pip install safety"
            )
        ...
        report = {
            ...
            "summary": {
                ...
                "critical_updates": len(
                    [p for p in outdated if p.update_priority == "critical"]
                ),
        ...
        if not dry_run:
            ...
            for package in safe_updates:
                try:
                    ...
                    results["errors"].append({"package": package.name, "error": str(e)})
        else:
    ...
    def _determine_severity(self, vuln_data: dict) -> str:
        ...
        if any(
            ...
            return "critical"
        if any(word in advisory for word in ["high", "sql injection", "xss"]):
            return "high"
        if any(word in advisory for word in ["medium", "denial of service"]):
            return "medium"
        return "low"
    ...
    def _determine_update_priority(self, pkg_data: dict) -> str:
        ...
        if pkg_name in self.sensitive_packages:
            return "medium"
        return "low"
    ...
    def _check_dependency_conflicts(self) -> list[dict]:
        ...
        try:
            result = subprocess.run(
                ["pip", "check"], check=False, capture_output=True, text=True
            )
    ...
    def _audit_requirements_file(self) -> list[dict]:
        ...
        if not self.requirements_file.exists():
            issues.append(
                {"type": "missing_file", "message": "Отсутствует файл requirements.txt"}
            )
            return issues
    ...
    def _generate_security_recommendations(
        self, vulnerabilities: list, outdated: list
    ) -> list[str]:
    ...
    def _backup_requirements(self):
        ...
        if self.requirements_file.exists():
            backup_file = (
                self.results_dir
                / f"requirements_backup_{int(datetime.now().timestamp())}.txt"
            )
            # Читать/писать в UTF-8
            backup_file.write_text(
                self.requirements_file.read_text(encoding="utf-8"), encoding="utf-8"
            )

    def _log_update_results(self, results: dict):
        """Логирует результаты обновлений"""
        if self.update_log.exists():
            with open(self.update_log, encoding="utf-8") as f:
                data = json.load(f)
        else:
            data = []
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--audit":
        ...
        print(
            f"Уязвимости: {report['summary']['total_vulnerabilities']} (критических: {report['summary']['critical_vulnerabilities']})"
        )
