...
class ImprovedSpotifyEnhancer(SpotifyEnhancer):
    ...
    def __init__(
        self, client_id: str = None, client_secret: str = None, db_path: str = None
    ):
    ...
    def _make_request(self, endpoint: str, params: dict = None) -> dict | None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Сетевая ошибка (попытка {attempt + 1}/{max_retries}): {type(e).__name__}: {str(e)[:100]}"
                )
    ...
    def search_track_improved(self, track_name: str, artist_name: str) -> Any | None:
        ...
        print("      1️⃣ Пробуем стратегию 'exact'...")
        ...
        if result:
            ...
            print("      ✅ Найдено стратегией 'exact'!")
            return result

        print("      🔄 Exact не сработал, пробуем другие стратегии...")
        ...
        if basic_track != clean_track:
            ...
            if result:
                logger.debug(
                    f"Найдено стратегией 'basic': {track_name} - {artist_name}"
                )
                self.stats["strategies_used"]["basic"] += 1
                print("      ✅ Найдено стратегией 'basic'!")
                return result

        # Стратегия 3: убираем feat/ft из названия
        no_feat_track = re.sub(
            r"\s+(feat\.|ft\.|featuring).*$", "", basic_track, flags=re.IGNORECASE
        )
        if no_feat_track != basic_track:
            ...
            if result:
                logger.debug(
                    f"Найдено стратегией 'no_feat': {track_name} - {artist_name}"
                )
                self.stats["strategies_used"]["no_feat"] += 1
                print("      ✅ Найдено стратегией 'no_feat'!")
                return result
        ...
        if keywords:
            ...
            if result:
                logger.debug(
                    f"Найдено стратегией 'keywords': {track_name} - {artist_name}"
                )
                self.stats["strategies_used"]["keywords"] += 1
                print("      ✅ Найдено стратегией 'keywords'!")
                return result

        # Стратегия 5: простой поиск без кавычек
        print("      5️⃣ Пробуем стратегию 'simple'...")
        ...
        if result:
            ...
            print("      ✅ Найдено стратегией 'simple'!")
            return result

        # Ничего не найдено
        print("      ❌ Трек не найден ни одной стратегией")
        self.stats["strategies_used"]["not_found"] += 1

        return None

    def _try_search_strategy(
        self, track_name: str, artist_name: str, strategy: str
    ) -> Any | None:
        """Попытка поиска с определенной стратегией"""
        try:
            if strategy == "exact" or strategy == "basic" or strategy == "no_feat":
            ...
            if "tracks" not in data:
                print("        ❌ Нет треков в ответе API")
                return None

            tracks = data["tracks"]["items"]
            if not tracks:
                print("        ❌ Пустой список треков")
                return None
            ...
            best_match = self._find_best_match(
                tracks, track_name, artist_name, strategy
            )
            if best_match:
                print(
                    f"        ✅ Найден match: {best_match.get('name', 'Unknown')} by {best_match.get('artists', [{}])[0].get('name', 'Unknown')}"
                )
    ...
    def _clean_track_name(self, track_name: str) -> str:
        ...
        clean = re.sub(
            r"^(official\s+)?(music\s+)?(video\s+)?", "", clean, flags=re.IGNORECASE
        )
        clean = re.sub(
            r"\s+(official|music|video|audio|lyric|lyrics)(\s+video|\s+audio)?$",
            "",
            clean,
            flags=re.IGNORECASE,
        )
        ...
        clean = re.sub(
            r"\s*\((hq|hd|4k|official|clean|explicit|radio\s+edit)\)\s*",
            " ",
            clean,
            flags=re.IGNORECASE,
        )
    ...
    def _get_basic_track_name(self, track_name: str) -> str:
        ...
        basic = re.sub(
            r"\s+(feat\.|ft\.|featuring)\s+", " feat ", basic, flags=re.IGNORECASE
        )
    ...
    def _find_best_match(
        ...
        def similarity_score(track_data: dict) -> float:
            ...
            artist_match = any(
                target_artist_lower in artist or artist in target_artist_lower
...
class ImprovedBulkSpotifyEnhancement:
        ...
        try:
            for i, (song_id, title, artist) in enumerate(tracks, 1):
                ...
                try:
                    ...
                    success_rate = self.stats["tracks_success"] / max(
                        self.stats["tracks_processed"], 1
                    )
            ...
            if "i" in locals() and i > 0:
                ...
                print("⚠️ Прерывание произошло до начала обработки")
        except Exception as e:
    ...
    def _process_track(self, song_id: int, title: str, artist: str) -> dict[str, Any]:
        ...
        try:
            ...
            error_type = (
                "network_error" if "connection" in str(e).lower() else "unknown_error"
            )
    ...
    def _show_progress(self, current: int, total: int, song_id: int):
        ...
        if self.stats["tracks_processed"] > 0:
            success_rate = (
                self.stats["tracks_success"] / self.stats["tracks_processed"] * 100
            )
