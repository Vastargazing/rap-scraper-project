...
class SpotifyEnhancementAnalyzer:
    ...
    async def get_detailed_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self.db.execute_query(
                "SELECT COUNT(*) as enhanced FROM tracks WHERE spotify_data IS NOT NULL"
            )
            stats["spotify_tracks"] = result[0]["enhanced"]

            result = await self.db.execute_query(
                "SELECT COUNT(DISTINCT artist) as artists FROM tracks WHERE spotify_data IS NOT NULL"
            )
            ...
            if stats["total_songs"] > 0:
                stats["enhancement_percentage"] = round(
                    (stats["spotify_tracks"] / stats["total_songs"]) * 100, 2
                )
            else:
            ...
            stats["top_unprocessed_artists"] = [
                (row["artist"], row["tracks_count"]) for row in result
            ]
            ...
            stats["top_processed_artists"] = [
                (row["artist"], row["processed_tracks"]) for row in result
            ]
            ...
            stats["most_popular_tracks"] = [
                (row["title"], row["artist"], row["popularity"]) for row in result
            ]
    ...
    async def find_problematic_tracks(self) -> dict[str, list]:
        ...
        try:
            ...
            long_titles = [
                (row["id"], row["title"], row["artist"], row["title_length"])
            ...
            special_chars = [
                (row["id"], row["title"], row["artist"]) for row in special_result
            ]

        finally:
            await self.db.close()

        return {"long_titles": long_titles, "special_characters": special_chars}
    ...
    def get_artist_coverage_report(self) -> dict[str, Any]:
        ...
        for artist, total, processed, percent in coverage_data:
            if percent == 0:
                coverage_groups["no_coverage"].append(
                    (artist, total, processed, percent)
                )
            elif percent <= 25:
                coverage_groups["low_coverage"].append(
                    (artist, total, processed, percent)
                )
            elif percent <= 75:
                coverage_groups["medium_coverage"].append(
                    (artist, total, processed, percent)
                )
            elif percent < 100:
                coverage_groups["high_coverage"].append(
                    (artist, total, processed, percent)
                )
            else:
                coverage_groups["full_coverage"].append(
                    (artist, total, processed, percent)
                )
...
def main():
    ...
    if "popularity" in stats:
        print("📊 Популярность треков:")
        print(f"   • Средняя: {stats['popularity']['avg']}")
        print(
            f"   • Мин-Макс: {stats['popularity']['min']}-{stats['popularity']['max']}"
        )
    ...
    print("🔥 Приоритетные артисты (много необработанных треков):")
