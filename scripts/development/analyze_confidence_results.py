...
def analyze_confidence_results():
    ...
    # Читаем результаты
    with open("confidence_test_updated.json", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for result in data["results"]:
        ...
        if confidences["metaphor"] < 0.5:
            recommendations.append(
                "⚠️  Низкая уверенность в детекции метафор - требуется ручная проверка"
            )

        if confidences["wordplay"] < 0.5:
            recommendations.append(
                "⚠️  Игра слов может быть недооценена - сложная детекция"
            )

        if confidences["rhyme_detection"] < 0.7:
            recommendations.append(
                "⚠️  Рифмы могут быть пропущены - проверьте сложные схемы"
            )
        ...
        print("\n📈 КЛЮЧЕВЫЕ МЕТРИКИ:")
        print(
            f"   Metaphor Count: {features.get('metaphor_count', 0)} (confidence: {confidences['metaphor']:.3f})"
        )
        print(
            f"   Wordplay Instances: {features.get('wordplay_instances', 0)} (confidence: {confidences['wordplay']:.3f})"
        )
        print(
            f"   Rhyme Density: {features.get('rhyme_density', 0):.3f} (confidence: {confidences['rhyme_detection']:.3f})"
        )
...
def show_best_practices():
    ...
    print("   • Фильтрация данных по минимальному confidence")
    print("   • Weighted loss functions на основе confidence")
    print("   • Uncertainty-aware predictions")
    print("   • Автоматическое лабелирование с high confidence")
...
if __name__ == "__main__":
    try:
        ...
        print("✅ Добавлены confidence scores для:")
        print("   • Rhyme detection & scheme analysis")
        print("   • Metaphor & wordplay detection")
        print("   • Flow & stress pattern analysis")
        print("   • Creativity assessment")
        print("\n📦 Интегрировано в:")
        print("   • SimplifiedFeatureAnalyzer")
        print("   • CLI экспорт (JSON/CSV)")
        print("   • Pydantic модели данных")

    except FileNotFoundError:
        print("❌ Файл confidence_test_updated.json не найден")
        print(
            "Запустите сначала: python scripts/rap_scraper_cli.py mlfeatures --batch 2 --export json --output confidence_test_updated.json"
        )
