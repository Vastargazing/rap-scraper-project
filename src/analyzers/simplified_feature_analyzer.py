...
class AnalysisProgress:
    ...
    def calculate_eta(self, remaining_tracks: int) -> Optional[datetime]:
...
def setup_logging(log_level: str = "INFO", log_file: Optional[str] = None) -> logging.Logger:
    ...
    # File handler
    if not log_file:
        log_file = log_dir / f"lyrics_analyzer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class LyricsAnalyzer:
    ...
    def __init__(self):
        ...
        self.stop_words = {
            ...
            'yeah', 'uh', 'got', 'get', 'gotta', 'wanna', 'gonna', 'ain', 'yall', 'em', 'ya'
        }

    def analyze(self, lyrics: str, track_id: Optional[int] = None) -> LyricsFeatures:
        """Perform comprehensive lyrics analysis with timing"""
        start_time = time.time()
        
        try:
            self.logger.debug("Starting analysis", extra={'track_id': track_id, 'lyrics_length': len(lyrics)})
            ...
            self.logger.debug("Analysis completed", extra={
                'track_id': track_id,
                'processing_time_ms': processing_time,
    ...
    def _preprocess_lyrics(self, lyrics: str) -> List[str]:
        ...
        for line in lines:
            ...
            if not re.match(r'^\[.*\]$|^\(.*\)$|^Verse \d+|^Chorus|^Bridge|^Outro', line, re.IGNORECASE):
    ...
    def _calculate_alliteration(self, lines: List[str]) -> float:
        ...
        for line in lines:
            words = [word.lower() for word in line.split() if len(word) >= 3 and word.isalpha()]
    ...
    def _analyze_flow(self, lines: List[str], words: List[str]) -> Dict:
        ...
        if len(line_syllable_counts) > 1:
            ...
            variance = sum((x - mean_syllables) ** 2 for x in line_syllable_counts) / len(line_syllable_counts)
            flow_consistency = max(0, 1 - (variance / (mean_syllables + 1)))
        else:
            flow_consistency = 1.0
        
        # Count flow breaks (punctuation that interrupts flow)
        flow_breaks = sum(line.count(',') + line.count('.') + line.count('!') + line.count('?') + line.count(';') for line in lines)
    ...
    def _calculate_composite_scores(self, rhyme_features: Dict, vocabulary_features: Dict, 
                                  creativity_features: Dict, flow_features: Dict) -> Dict:
...
class PostgreSQLManager:
    ...
    def __init__(self, config: Optional[DatabaseConfig] = None):
        ...
        if config_errors:
            raise ValueError(f"Database configuration errors: {', '.join(config_errors)}")

    async def initialize(self) -> bool:
        """Initialize connection pool with comprehensive error handling"""
        try:
            self.logger.info("Initializing PostgreSQL connection pool", extra={
            ...
            self.logger.info("PostgreSQL connection pool initialized successfully", extra={
            ...
            self.logger.error("Failed to initialize PostgreSQL connection pool", extra={
                'error': str(e),
                'host': self.config.host,
                'port': self.config.port,
                'database': self.config.database
        ...
        try:
            ...
            if result:
                ...
                if stats['total_tracks'] and stats['analyzed_tracks']:
                    stats['analysis_percentage'] = (stats['analyzed_tracks'] / stats['total_tracks']) * 100
                else:
...
class AnalysisEngine:
    ...
    async def initialize(self) -> bool:
        ...
        self.logger.info("Analysis engine initialized successfully", extra={
        ...
        try:
            if self.progress_file.exists():
                ...
                if datetime.now() - last_update > timedelta(hours=24):
                    self.logger.info("Starting new session (previous session > 24h old)")
                    return self._create_new_progress()
                
                self.logger.info("Loaded existing progress", extra={
    ...
    def _create_new_progress(self) -> AnalysisProgress:
        ...
        self.logger.info("Created new analysis session", extra={'session_id': session_id})
        return progress
    ...
    def _save_progress(self):
        ...
        try:
            ...
            if self.current_progress.total_processed > 0:
                session_start = datetime.fromisoformat(self.current_progress.session_start)
                elapsed_seconds = (datetime.now() - session_start).total_seconds()
                self.current_progress.processing_rate = self.current_progress.total_processed / max(elapsed_seconds, 1)
            ...
            self.logger.debug("Progress saved successfully", extra={
                'last_processed_id': self.current_progress.last_processed_id,
    ...
    async def analyze_all_tracks(self, batch_size: int = 50, max_tracks: Optional[int] = None) -> Dict:
        """Analyze all unprocessed tracks with comprehensive progress tracking"""
        
        self.logger.info("Starting full database analysis", extra={
        ...
        try:
            while not self._shutdown_requested:
                ...
                self.logger.info(f"Processing batch {batch_number}", extra={
                ...
                self.current_progress.batch_statistics['successful_batches'] += 1 if batch_results['processed'] > 0 else 0
                self.current_progress.batch_statistics['failed_batches'] += 1 if batch_results['errors'] > 0 else 0
                
                # Save progress
                self._save_progress()
                
                # Log progress
                self.logger.info("Batch completed", extra={
    ...
    async def _get_next_batch(self, batch_size: int) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error("Failed to get next batch", extra={
                'last_processed_id': self.current_progress.last_processed_id,
                'batch_size': batch_size
            }, exc_info=True)
            return []
    ...
    async def _process_batch(self, tracks: List[Dict]) -> Dict[str, int]:
            ...
            async with self.db_manager.get_connection() as conn:
                async with conn.transaction():
                    for track in tracks:
                        ...
                        try:
                            # Analyze track
                            features = self.analyzer.analyze(track['lyrics'], track['id'])
                            ...
                            self.session_stats['total_processing_time'] += features.processing_time_ms
                            ...
                            self.logger.warning("Track processing failed", extra=error_info)
        ...
        self.logger.debug("Batch processing completed", extra={
            'processed': processed,
            'errors': errors,
    ...
    async def _save_analysis_result(self, conn: asyncpg.Connection, track: Dict, features: LyricsFeatures):
    ...
    async def close(self):
        ...
        self.logger.info("Analysis engine shutdown complete", extra={
...
async def main():
    ...
    logger.info("Starting Professional Lyrics Analyzer v2.0", extra={
    ...
    try:
        ...
        try:
            ...
            if results.get('shutdown_requested'):
                print(f"  Status: Gracefully shutdown (SIGINT/SIGTERM)")
            else:
