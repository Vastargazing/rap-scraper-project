...
def test_confidence_scores():
    ...
    for text_type, lyrics in test_texts.items():
        ...
        print(
            f"   ✅ Rhyme Detection Confidence: {features.rhyme_analysis.rhyme_detection_confidence:.3f}"
        )
        print(f"   Rhyme Scheme: {features.rhyme_analysis.end_rhyme_scheme}")
        print(
            f"   ✅ Rhyme Scheme Confidence: {features.rhyme_analysis.rhyme_scheme_confidence:.3f}"
        )

        print("\n🎭 METAPHOR ANALYSIS:")
        print(f"   Metaphor Count: {features.metaphor_analysis.metaphor_count}")
        print(
            f"   ✅ Metaphor Confidence: {features.metaphor_analysis.metaphor_confidence:.3f}"
        )
        print(f"   Wordplay Instances: {features.metaphor_analysis.wordplay_instances}")
        print(
            f"   ✅ Wordplay Confidence: {features.metaphor_analysis.wordplay_confidence:.3f}"
        )
        print(f"   Creativity Score: {features.metaphor_analysis.creativity_score:.3f}")
        print(
            f"   ✅ Creativity Confidence: {features.metaphor_analysis.creativity_confidence:.3f}"
        )

        print("\n🌊 FLOW ANALYSIS:")
        print(f"   Syllable Count: {features.flow_analysis.syllable_count}")
        print(
            f"   Stress Pattern Consistency: {features.flow_analysis.stress_pattern_consistency:.3f}"
        )
        print(
            f"   ✅ Stress Pattern Confidence: {features.flow_analysis.stress_pattern_confidence:.3f}"
        )
        print(
            f"   ✅ Flow Analysis Confidence: {features.flow_analysis.flow_analysis_confidence:.3f}"
        )
...
def demonstrate_json_output():
    ...
    key_metrics = {
        "metaphor_analysis": {
            ...
            "metaphor_confidence": result_dict["metaphor_analysis"][
                "metaphor_confidence"
            ],
            "wordplay_instances": result_dict["metaphor_analysis"][
                "wordplay_instances"
            ],
            "wordplay_confidence": result_dict["metaphor_analysis"][
                "wordplay_confidence"
            ],
        },
        "rhyme_analysis": {
            "rhyme_density": result_dict["rhyme_analysis"]["rhyme_density"],
            "rhyme_detection_confidence": result_dict["rhyme_analysis"][
                "rhyme_detection_confidence"
            ],
            "rhyme_scheme_confidence": result_dict["rhyme_analysis"][
                "rhyme_scheme_confidence"
            ],
        },
        "flow_analysis": {
            "stress_pattern_confidence": result_dict["flow_analysis"][
                "stress_pattern_confidence"
            ],
            "flow_analysis_confidence": result_dict["flow_analysis"][
                "flow_analysis_confidence"
            ],
        },
    }
