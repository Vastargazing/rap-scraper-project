...
class RapDatasetPreparator:
    ...
    def parse_spotify_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            if pd.notna(row["spotify_data"]):
                try:
                    spotify_json = (
                        json.loads(row["spotify_data"])
                        if isinstance(row["spotify_data"], str)
                        else row["spotify_data"]
                    )
                    ...
                    if "audio_features" in spotify_json:
                        ...
                        for feature in spotify_features:
                            if feature in audio_features:
                                df.at[idx, f"spotify_{feature}"] = audio_features[
                                    feature
                                ]
                    ...
                    if "artist_info" in spotify_json:
                        ...
                        df.at[idx, "artist_popularity"] = artist_info.get(
                            "popularity", np.nan
                        )
                        df.at[idx, "artist_followers"] = artist_info.get(
                            "followers", {}
                        ).get("total", np.nan)
                        df.at[idx, "artist_genres"] = ", ".join(
        ...
        df["artist_popularity"] = df["artist_popularity"].fillna(
            df["artist_popularity"].median()
        )
        df["artist_followers"] = df["artist_followers"].fillna(
            df["artist_followers"].median()
        )
        df["artist_genres"] = df["artist_genres"].fillna("unknown")

        spotify_count = df["spotify_data"].notna().sum()
        logger.info(
            f"✅ Parsed Spotify features for {spotify_count} tracks ({spotify_count / len(df) * 100:.1f}%)"
        )
    ...
    def parse_ai_analysis_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            if pd.notna(row["qwen_analysis"]):
                try:
                    qwen_data = (
                        json.loads(row["qwen_analysis"])
                        if isinstance(row["qwen_analysis"], str)
                        else row["qwen_analysis"]
                    )
                    ...
                    if "emotion_intensity" in qwen_data:
                        df.at[idx, "qwen_emotion_intensity"] = qwen_data[
                            "emotion_intensity"
                        ]
            ...
            if pd.notna(row["algo_features"]):
                try:
                    algo_data = (
                        json.loads(row["algo_features"])
                        if isinstance(row["algo_features"], str)
                        else row["algo_features"]
                    )
                    ...
                    if "vocabulary_diversity" in algo_data:
                        df.at[idx, "vocab_diversity"] = algo_data[
                            "vocabulary_diversity"
                        ]
                    if "sentiment_score" in algo_data:
                        df.at[idx, "algo_sentiment_score"] = algo_data[
                            "sentiment_score"
                        ]
        ...
        df["consensus_complexity"] = df[["qwen_complexity", "gemma_complexity"]].mean(
            axis=1, skipna=True
        )
        ...
        ai_features = [
            "qwen_quality",
            "qwen_emotion_intensity",
            "qwen_wordplay",
            "rhyme_density",
            "vocab_diversity",
            "algo_sentiment_score",
        ]
    ...
    def engineer_text_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["words_per_line_norm"] = (
            df["avg_words_per_line"] / df["avg_words_per_line"].max()
        )
    ...
    def create_text_embeddings(
        self, df: pd.DataFrame, sample_size: int = 5000
    ) -> pd.DataFrame:
        ...
        logger.info(
            f"✅ Text embeddings created ({embeddings_reduced.shape[1]} dimensions)"
        )
        return df
    ...
    def create_style_labels(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"📊 Identified {len(top_artists)} major artists for style classification"
        )
        ...
        def categorize_themes(themes_str):
            ...
            if any(
                word in themes_lower for word in ["love", "romance", "relationship"]
            ):
                return "love"
            if any(
                word in themes_lower for word in ["money", "success", "wealth", "fame"]
            ):
                return "success"
            if any(
                word in themes_lower
                for word in ["struggle", "pain", "poverty", "hardship"]
            ):
                return "struggle"
            if any(
                word in themes_lower
                for word in ["party", "club", "dance", "celebration"]
            ):
                return "party"
            if any(
                word in themes_lower
                for word in ["social", "society", "justice", "politics"]
            ):
        ...
        logger.info("\n📊 Label Distributions:")
        logger.info(
            f"Artist Styles: {df['artist_style'].value_counts().head(10).to_dict()}"
        )
        ...
        logger.info(
            f"Theme Categories: {df['theme_category'].value_counts().to_dict()}"
        )
        logger.info(
            f"Quality Score - Mean: {df['quality_score'].mean():.3f}, Std: {df['quality_score'].std():.3f}"
        )
        logger.info(
            f"Commercial Potential - Mean: {df['commercial_potential'].mean():.3f}, Std: {df['commercial_potential'].std():.3f}"
        )
    ...
    def prepare_final_dataset(self, df: pd.DataFrame) -> dict:
        ...
        # Embeddings
        embedding_features = [
            f"embedding_{i}" for i in range(50) if f"embedding_{i}" in df.columns
        ]
        ...
        for cat_feature in categorical_features:
            if cat_feature in df.columns:
                ...
                df[encoded_col] = self.label_encoders[cat_feature].fit_transform(
                    df[cat_feature].fillna("unknown")
                )
        ...
        logger.info(
            f"   Spotify Coverage: {metadata['data_statistics']['spotify_coverage']:.1%}"
        )
        logger.info(
            f"   Qwen Coverage: {metadata['data_statistics']['qwen_coverage']:.1%}"
        )
...
async def main():
    ...
    try:
        ...
        print(
            f"🎵 Spotify Coverage: {ml_dataset['metadata']['data_statistics']['spotify_coverage']:.1%}"
        )
        print(
            f"🤖 Qwen Coverage: {ml_dataset['metadata']['data_statistics']['qwen_coverage']:.1%}"
        )
        print(
            f"💎 Average Quality Score: {ml_dataset['y']['quality_score'].mean():.3f}"
        )
        print(
            f"💰 Average Commercial Potential: {ml_dataset['y']['commercial_potential'].mean():.3f}"
        )
