...
class RapQualityPredictor:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def calculate_rhyme_density(lyrics):
            ...
            for i in range(len(lines) - 1):
                ...
                line1_end = (
                    lines[i].split()[-1][-2:].lower() if lines[i].split() else ""
                )
                line2_end = (
                    lines[i + 1].split()[-1][-2:].lower()
                    if lines[i + 1].split()
                    else ""
                )
        ...
        features_df["vocabulary_richness"] = df["lyrics"].apply(
        ...
        features_df["qwen_confidence"] = pd.to_numeric(
            df["qwen_confidence"], errors="coerce"
        ).fillna(0.7)
        features_df["qwen_complexity"] = pd.to_numeric(
            df["qwen_complexity"], errors="coerce"
        ).fillna(0.5)
                    ...
                    features.update(
                        {
                            ...
                            "artist_followers": np.log1p(
                                artist.get("followers", {}).get("total", 10000)
                            )
                            / 20,
                        }
                    )
        ...
        spotify_features_list = (
            df["spotify_data"].apply(extract_spotify_features).tolist()
        )
        ...
        features_df["artist_style_encoded"] = self.label_encoders[
            "artist_style"
        ].fit_transform(df["artist_style"].fillna("other"))

        # Theme category encoding
        if "theme_category" not in self.label_encoders:
            self.label_encoders["theme_category"] = LabelEncoder()

        features_df["theme_category_encoded"] = self.label_encoders[
            "theme_category"
        ].fit_transform(df["theme_category"].fillna("general"))

        # Sentiment encoding
        if "qwen_sentiment" not in self.label_encoders:
            self.label_encoders["qwen_sentiment"] = LabelEncoder()

        features_df["sentiment_encoded"] = self.label_encoders[
            "qwen_sentiment"
        ].fit_transform(df["qwen_sentiment"].fillna("neutral"))

        # === INTERACTION FEATURES ===
        # Readability vs Complexity
        features_df["readability_complexity_ratio"] = features_df[
            "avg_words_per_line"
        ] / (features_df["qwen_complexity"] + 0.1)
        ...
        logger.info(
            f"✅ Feature engineering completed: {features_df.shape[1]} features"
        )
    ...
    def create_target_variables(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        targets_df["commercial_potential"] = df.apply(
            calculate_commercial_potential, axis=1
        )
        ...
        def calculate_viral_potential(row):
            ...
            if hasattr(df, "lyrics") and row.name in df.index:
                repetitive_ratio = (
                    targets_df.loc[row.name : row.name, "repetitive_ratio"].iloc[0]
                    if "repetitive_ratio" in targets_df.columns
                    else 0
                )
            ...
            length_viral_score = (
                max(0, 1 - (word_count - 200) / 400) if word_count > 200 else 1
            )
        ...
        targets_df["longevity_score"] = (
            ...
            + base_quality * 0.3
            + (1 - targets_df["commercial_potential"])
            * 0.2  # Less commercial = more artistic
        )
    ...
    def train_models(
        ...
        try:
            ...
            target_models = {
                "quality_score": GradientBoostingRegressor(
                    n_estimators=100, learning_rate=0.1, max_depth=6, random_state=42
                ),
                "commercial_potential": RandomForestRegressor(
                    n_estimators=100, max_depth=8, random_state=42
                ),
                "viral_potential": GradientBoostingRegressor(
                    n_estimators=100, learning_rate=0.15, max_depth=5, random_state=42
                ),
                "longevity_score": RandomForestRegressor(
            ...
            for target_name, model in target_models.items():
                ...
                cv_scores = cross_val_score(
                    model, X_scaled, y[target_name], cv=cv_folds, scoring="r2"
                )
            ...
            if hasattr(model, "feature_importances_"):
                for feature, importance in zip(
                    ...
                    importance_sum[feature] = (
                        importance_sum.get(feature, 0) + importance
                    )

        # Sort by importance
        sorted_features = sorted(
            importance_sum.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def predict_quality_metrics(
        ...
        try:
            ...
            dummy_data["avg_words_per_line"] = [
                dummy_data["word_count"][0] / dummy_data["lines_count"][0]
            ]
            ...
            return dict.fromkeys(self.models.keys(), 0.5)
...
def train_quality_models():
    ...
    try:
        ...
        results = predictor.train_models(
            dataset_path="data/ml/quick_dataset.pkl", test_size=0.2, cv_folds=5
        )
        ...
        logger.info(
            "  scores = predictor.predict_quality_metrics(lyrics, artist_style, theme, sentiment)"
        )
...
def demo_quality_prediction():
    ...
    try:
        ...
        for track in demo_tracks:
            ...
            print("=" * 60)
            print(
                f"Style: {track['style']} | Theme: {track['theme']} | Sentiment: {track['sentiment']}"
            )
            print(f"\nLyrics:\n{track['lyrics'].strip()}")
