...
class EnhancedFileContext:
    ...
    semantic_embedding: Optional[List[float]] = None  # Для семантического поиска
    ...
    def generate_descriptions(self, file_contexts: Dict) -> Dict[str, str]:
        ...
        for file_path, context in file_contexts.items():
            try:
                ...
                import asyncio
                description = asyncio.run(self.generate_file_description(Path(file_path)))
                descriptions[file_path] = description
                print(f"✅ {file_path}: {description[:50]}...")
            except Exception as e:
                print(f"❌ Ошибка для {file_path}: {e}")
                descriptions[file_path] = self._fallback_description(Path(file_path), context.content[:500])
...
class DependencyVisualizer:
    ...
    def generate_dependency_graph(self, focus_files: Optional[List[str]] = None) -> str:
        ...
        # Фильтруем файлы
        files_to_show = focus_files if focus_files else list(self.file_contexts.keys())[:20]
    ...
    def save_graph(self, focus_category: Optional[str] = None) -> Path:
        ...
        print(f"💡 Для рендеринга: dot -Tsvg {dot_file} -o {dot_file.with_suffix('.svg')}")
...
class SimpleAPI:
    ...
    def _setup_routes(self):
        ...
        @self.app.get("/files")
        async def list_files(category: Optional[str] = None, min_priority: float = 0.0):
        ...
        async def get_stats():
            ...
            return {
                ...
                "avg_complexity": round(total_complexity / len(self.context_manager.file_contexts), 1),
        ...
        async def visualize_category(category: str):
            ...
            dot_content = visualizer.generate_dependency_graph(
                focus_files=[
                    path for path, ctx in self.context_manager.file_contexts.items()
...
class AIContextManagerPro:
    ...
    def setup_advanced_features(self, enable_llm: bool = False, enable_api: bool = False):
        ...
        try:
            ...
            try:
                ...
                if analyzer_results_file.exists():
                    with open(analyzer_results_file, 'r', encoding='utf-8') as f:
                    ...
                    with open(analyzer_results_file, 'w', encoding='utf-8') as f:
                        json.dump(self.analyzer_metrics, f, indent=2, ensure_ascii=False)
    ...
    def _enhance_context_with_analyzer_data(self, file_path: str, context: EnhancedFileContext):
        ...
        if file_path in file_metrics:
            ...
            if 'complexity_score' in metrics:
                context.complexity_score = max(context.complexity_score, metrics['complexity_score'])
    ...
    def _calculate_coupling(self, file_path: Path) -> float:
        ...
        try:
            ...
            internal_imports = sum(1 for imp in imports if not imp.startswith(('python', 'sys', 'os')))
    ...
    def _generate_smart_summary(self, files: List[str], task_type: str, query: str) -> str:
        ...
        if avg_complexity > 50:
            summary_parts.append(f"⚠️ Высокая сложность кода (avg: {avg_complexity:.1f})")
    ...
    def _suggest_commands_smart(self, task_type: str, files: List[str], query: str) -> List[str]:
        ...
        if task_type == "debug":
            ...
            if has_analyzer:
                commands.append(f"python main.py --analyze '{query or 'test'}' --analyzer hybrid")
            commands.append("python scripts/tools/ai_project_analyzer.py  # Аудит архитектуры")
    ...
    def _generate_warnings_smart(self, task_type: str, files: List[str]) -> List[str]:
        ...
        legacy_count = sum(1 for f in files if "legacy" in self.file_contexts.get(f, EnhancedFileContext("", 0, "", "", "", 0, [])).category)
        if legacy_count > 0:
            warnings.append(f"⚠️ {legacy_count} legacy файлов в контексте - используй только для справки")
        
        # Проверяем сложность
        high_complexity_files = [
            f for f in files 
            if self.file_contexts.get(f, EnhancedFileContext("", 0, "", "", "", 0, [])).complexity_score > 100
        ]
        if high_complexity_files:
            warnings.append(f"🔥 {len(high_complexity_files)} файлов с высокой сложностью - будь внимателен")
        
        # Проверяем недавние изменения
        recent_critical = [
            f for f in files
            if self.file_contexts.get(f) and 
            self.file_contexts[f].priority >= 4.0 and
            "hour" in self.file_contexts[f].git_last_commit
        ]
        if recent_critical:
            warnings.append(f"🚨 {len(recent_critical)} критичных файлов изменены недавно - проверь совместимость")
    ...
    def _generate_ml_insights(self, files: List[str], query: str) -> List[str]:
        ...
        if len(files) > 10:
            ...
            most_coupled = sorted(
                [(f, self.file_contexts[f].coupling_score) for f in files if f in self.file_contexts],
                key=lambda x: x[1],
                reverse=True
            )[:3]
        ...
        if len(active_authors) > 3:
            insights.append(f"👥 Над этими файлами работали {len(active_authors)} разработчиков")
    ...
    def _determine_category(self, file_path: Path) -> str:
        ...
        if 'database' in path_str or 'postgres' in path_str:
            return 'database'
        elif 'analyzer' in path_str:
            return 'analyzer'
        elif 'cli' in path_str or file_path.name == 'main.py':
            return 'cli'
        elif 'model' in path_str:
            return 'models'
        elif 'test' in path_str:
            return 'tests'
        elif 'archive' in path_str or 'sqlite' in path_str:
            return 'legacy'
    ...
    def _generate_description(self, file_path: Path) -> str:
        ...
        try:
            ...
            if '"""' in first_lines:
                docstring = first_lines.split('"""')[1] if len(first_lines.split('"""')) > 1 else ""
...
def auto_detect_task_type(query: str) -> str:
    ...
    debug_keywords = ['error', 'bug', 'fix', 'broken', 'crash', 'timeout', 'exception', 'fail']
    ...
    if any(kw in query_lower for kw in debug_keywords):
        return "debug"
    elif any(kw in query_lower for kw in develop_keywords):
        return "develop"
    elif any(kw in query_lower for kw in analyze_keywords):
        return "analyze"
    elif any(kw in query_lower for kw in refactor_keywords):
        return "refactor"
    else:
    ...
    while True:
        ...
        if choice == "0":
            ...
            if query:
                ...
                if export == 'y':
                    export_context(context, f"context_{task_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
            ...
            if success:
                print("✅ Интеграция успешна! Данные Project Analyzer добавлены в контекст")
                manager._update_dynamic_data()
                manager.save_context()
            else:
                print("❌ Интеграция не удалась - проверьте наличие ai_project_analyzer.py")
...
def print_project_stats(manager: AIContextManagerPro):
    ...
    for category, contexts in categories.items():
        ...
        print(f"  • {category}: {len(contexts)} файлов (avg priority: {avg_priority:.1f})")
...
• Динамические метрики сложности
    ...
    parser.add_argument("--task", "-t", choices=["debug", "develop", "analyze", "refactor"],
                       help="Тип задачи")
    parser.add_argument("--query", "-q", type=str,
                       help="Описание задачи для автоопределения типа")
    parser.add_argument("--semantic-search", "-s", type=str,
                       help="Семантический поиск по файлам")
    parser.add_argument("--update-cache", "-u", action="store_true",
                       help="Обновить кеш без генерации контекста")
    parser.add_argument("--export", "-e", type=str,
                       help="Экспорт результата в файл")
    parser.add_argument("--integrate", action="store_true",
                       help="Интеграция с ai_project_analyzer")
    parser.add_argument("--stats", action="store_true",
                       help="Показать статистику проекта")
    
    # Advanced features
    parser.add_argument("--llm-descriptions", action="store_true",
                       help="Генерация AI описаний через LLM (Ollama)")
    parser.add_argument("--visualize", "-v", action="store_true",
                       help="Создать граф зависимостей")
    parser.add_argument("--api", action="store_true",
                       help="Запустить REST API сервер")
    ...
    if args.query:
        ...
        print("Используйте --help для списка команд или --interactive для интерактивного режима")
        print("\n🚀 Быстрый старт:")
