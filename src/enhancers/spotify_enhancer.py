...
class SpotifyEnhancer:
    ...
    def _setup_spotify_client(self):
        ...
        try:
            ...
            logger.info(f"📍 Перед созданием клиента: .cache в корне {'ЕСТЬ' if os.path.exists(root_cache) else 'НЕТ'}")
            ...
            logger.info(f"📍 После создания клиента: .cache в корне {'ЕСТЬ' if os.path.exists(root_cache) else 'НЕТ'}")
            logger.info(f"📍 После создания клиента: .cache в data/ {'ЕСТЬ' if os.path.exists(cache_path) else 'НЕТ'}")
            ...
            logger.info(f"📍 После API вызова: .cache в корне {'ЕСТЬ' if os.path.exists(root_cache) else 'НЕТ'}")
            logger.info(f"📍 После API вызова: .cache в data/ {'ЕСТЬ' if os.path.exists(cache_path) else 'НЕТ'}")
            
            # Перемещаем кеш если он создался в корне
            if os.path.exists(root_cache):
                logger.warning(f"⚠️ .cache создан в корне после API вызова! Перемещаем в {cache_path}")
            ...
            try:
                with open(root_cache, 'w') as f:
                    f.write("# Этот файл блокирует создание кеша в корне\n# Реальный кеш находится в data/.cache\n")
    ...
    async def get_tracks_for_enhancement(self, limit: int = 1000) -> List[Dict[str, Any]]:
    ...
    async def bulk_enhance(self, batch_size: int = 50, max_tracks: Optional[int] = None) -> Dict[str, int]:
        ...
        try:
            ...
            for i in range(0, len(tracks), batch_size):
                ...
                logger.info(f"Batch {i//batch_size + 1}/{(len(tracks)-1)//batch_size + 1}")
                
                for track in batch:
                    success = await self.enhance_song(track['id'], track['artist'], track['title'])
        ...
        while True:
            ...
            if choice == "1":
                ...
                if confirm == 'y':
                    ...
                    print(f"🔄 Запуск обогащения: батч={batch_size}, лимит={max_tracks or 'все'}")
                ...
                print(f"🔄 Запуск тестового обогащения...")
                stats = await enhancer.bulk_enhance(
                    batch_size=batch_size,
                    max_tracks=100
                )
