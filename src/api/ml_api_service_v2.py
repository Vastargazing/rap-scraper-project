...
class AnalyzeRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(None, description="Override config temperature")
...
async def root():
    ...
    return {
        ...
        "docs": f"{config.api.docs.swagger_url}" if config.api.docs.enabled else "disabled",
        "status": "running",
...
async def health_check():
    ...
    status = "healthy" if all(
        v in ["healthy", "unavailable"] for v in components.values()
    ) else "degraded"
...
async def config_info():
    ...
    return {
        ...
        "api": {
            ...
            "rate_limit_rpm": config.api.rate_limit.requests_per_minute if config.api.rate_limit.enabled else None
        },
        "database": {
...
async def analyze_lyrics(request: AnalyzeRequest):
    ...
    if not qwen_analyzer:
        raise HTTPException(
            status_code=503,
            detail="QWEN analyzer not available"
        )
    ...
    try:
        ...
        return {
            "success": True,
            "analysis": result,
            "cached": "timestamp" not in result or time.time() - result.get("timestamp", 0) > 60,
            "timestamp": datetime.now().isoformat()
        }
...
if __name__ == "__main__":
    ...
    logger.info(f"Docs: http://{config.api.host}:{config.api.port}{config.api.docs.swagger_url}")
    logger.info("=" * 60)
