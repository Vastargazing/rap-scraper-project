            ...
            async with self.db_manager.connection_pool.acquire() as conn:
                ...
                for record in records:
                    try:
                        ...
                        themes = json.loads(record['themes']) if record['themes'] else []
...
{{
    ...
    async def evaluate_model(self) -> Dict[str, Any]:
        ...
        for i, sample in enumerate(eval_samples, 1):
            print(f"🧪 Evaluating {i}/{len(eval_samples)}: {sample['artist']} - {sample['title']}")
        ...
        # Сохранение результатов
        eval_file = self.results_dir / f"evaluation_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
Примеры использования:
  ...
  python models/test_qwen.py --all                     # Полный цикл ML pipeline
    ...
    parser.add_argument('--train', action='store_true',
                       help='Запуск симуляции обучения модели')
    parser.add_argument('--evaluate', action='store_true',
                       help='Оценка модели на evaluation данных')
    parser.add_argument('--all', action='store_true',
                       help='Выполнить полный ML pipeline')
    ...
    try:
        ...
        if not any([args.test_api, args.prepare_dataset, args.train, args.evaluate, args.all]):
