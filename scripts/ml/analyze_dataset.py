...
print("\n=== SAMPLE DATA ===")
print(df[['artist', 'word_count', 'qwen_sentiment', 'artist_style', 'mood_label']].head())
...
print(f"Lyrics length >= 200: {criteria2.sum()}/{len(df)} ({criteria2.mean()*100:.1f}%)")
print(f"Has sentiment: {criteria3.sum()}/{len(df)} ({criteria3.mean()*100:.1f}%)")

all_criteria = criteria1 & criteria2 & criteria3
print(f"All criteria: {all_criteria.sum()}/{len(df)} ({all_criteria.mean()*100:.1f}%)")