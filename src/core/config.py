...
class AnalysisConfig:
    ...
    supported_analyzers: list[str] = field(
        default_factory=lambda: ["gemma", "algorithmic", "hybrid"]
    )
...
class AppConfig:
    ...
    def save_to_file(self, file_path: str | Path) -> None:
        ...
        if file_path.suffix.lower() == ".json":
            ...
            if not HAS_YAML:
                raise ValueError(
                    "PyYAML package required for YAML support. Install with: pip install PyYAML"
                )
            with open(file_path, "w", encoding="utf-8") as f:
...
class ConfigLoader:
    ...
    def load_from_env(self, prefix: str = "RAP_SCRAPER_") -> dict[str, Any]:
        ...
        if os.getenv(f"{prefix}DEBUG"):
            config["debug"] = os.getenv(f"{prefix}DEBUG").lower() in [
                "true",
                "1",
                "yes",
            ]
...
class ConfigManager:
    ...
    def _dict_to_appconfig(self, config_dict: dict[str, Any]) -> AppConfig:
        ...
        for key, value in config_dict.items():
            if hasattr(app_config, key) and key not in [
                "database",
                "scraping",
                "analysis",
                "logging",
    ...
    def save_current_config(self, file_path: str | Path) -> None:
