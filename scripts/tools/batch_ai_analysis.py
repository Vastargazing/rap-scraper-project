...
def main(
    ...
    logger.info("=" * 60)
    logger.info(
        f"📊 Config: db={db_path} | batch_size={batch_size} | sleep={sleep_sec}s | max_batches={max_batches}"
    )
    ...
    try:
        ...
        while True:
            ...
            progress = (
                f"({batches + 1}" + (f"/{max_batches}" if max_batches else "") + ")"
            )
            ...
            analyzer.batch_analyze_from_db(
                db_path=db_path, limit=current_batch, offset=0
            )
            ...
            logger.info(
                f"✅ Batch completed in {batch_time:.1f}s | {avg_time_per_song:.2f}s per song"
            )
