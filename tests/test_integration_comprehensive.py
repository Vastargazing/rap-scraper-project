            ...
            if hasattr(analyzer, "analyze_song"):
                ...
                try:
                    result = analyzer.analyze_song(
                        "Test Artist", "Test Song", "This is a test lyrics for analysis"
                    )
    ...
    async def test_analyzer_cli_basic_analysis(self):
        ...
        try:
            result = await cli.analyze_text(
                text=test_text, analyzer_type="algorithmic_basic"
            )
    ...
    async def test_end_to_end_analysis_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                result = await cli.analyze_text(
                    text=test_texts[0], analyzer_type="algorithmic_basic"
                )
                assert isinstance(result, dict)
...
class TestErrorHandling:
    ...
    async def test_invalid_analyzer_handling(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            await cli.analyze_text(
                text="Test text", analyzer_type="nonexistent_analyzer"
            )
    ...
    async def test_empty_text_handling(self):
        ...
        try:
            result = await cli.analyze_text(text="", analyzer_type="algorithmic_basic")
...
def run_basic_tests():
    ...
    pytest.main([__file__ + "::TestAnalyzerInfrastructure", "-v", "--tb=short"])


def run_cli_tests():
    """Запуск тестов CLI"""
    pytest.main([__file__ + "::TestCLIComponents", "-v", "--tb=short"])


def run_integration_tests():
    """Запуск интеграционных тестов"""
    pytest.main([__file__ + "::TestIntegrationScenarios", "-v", "--tb=short"])
