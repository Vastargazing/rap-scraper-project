...
from models import SpotifyArtist, SpotifyTrack, SpotifyEnrichmentResult
...
class TestSpotifyEnhancer(unittest.TestCase):
    ...
    def test_create_spotify_tables(self):
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='spotify_artists'")
        self.assertIsNotNone(cursor.fetchone())
        
        # Проверяем таблицу треков
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='spotify_tracks'")
        self.assertIsNotNone(cursor.fetchone())
        
        # Проверяем таблицу аудио-характеристик
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='spotify_audio_features'")
        self.assertIsNotNone(cursor.fetchone())
    ...
    def test_save_artist_to_db(self):
        ...
        conn = sqlite3.connect(self.enhancer.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM spotify_artists WHERE artist_name = ?", ("Test Artist",))
