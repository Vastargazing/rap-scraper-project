...
try:
    ...
    print(f"   Connection String: postgresql://{config.database.username}:***@{config.database.host}:{config.database.port}/{config.database.database_name}")
    ...
    print(f"\n   ⏰ Cache TTL:")
    print(f"      Artist: {config.redis.cache.artist_ttl}s ({config.redis.cache.artist_ttl // 3600}h)")
    print(f"      Lyrics: {config.redis.cache.lyrics_ttl}s ({config.redis.cache.lyrics_ttl // 3600}h)")
    print(f"      Analysis: {config.redis.cache.analysis_ttl}s ({config.redis.cache.analysis_ttl // 86400}d)")
    print(f"      Embedding: {config.redis.cache.embedding_ttl}s ({config.redis.cache.embedding_ttl // 86400}d)")
    ...
    print(f"      Default Metrics: {config.monitoring.prometheus.include_default_metrics}")
    print(f"\n   📊 Grafana:")
    ...
    print("   python -c \"from src.cache import test_redis_connection; test_redis_connection()\"")
