    ...
    def find_security_issues(self, file_metrics: Dict) -> List[str]:
        ...
        for file_path, metrics in file_metrics.items():
            try:
                ...
                # Insecure random
                if re.search(r'import random\b', content) and re.search(r'random\.(choice|randint|random)', content):
    ...
    def find_performance_issues(self, file_metrics: Dict) -> List[str]:
        ...
        for file_path, metrics in file_metrics.items():
            try:
                ...
                if nested_loops > 2:
                    issues.append(f"🔄 Deep nested loops (depth: {nested_loops}) in {file_path}")
                ...
                for op in inefficient_ops:
                    issues.append(f"⚡ Inefficient operation in loop: {op} in {file_path}")
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.Call):
                        if (isinstance(child.func, ast.Attribute) and 
                            child.func.attr in ['execute', 'query', 'get', 'filter']):
                            return True
                    ...
                    if isinstance(child, (ast.Str, ast.Constant)):
                        ...
                        if isinstance(str_value, str) and any(sql_word in str_value.upper() for sql_word in ['SELECT', 'INSERT', 'UPDATE', 'DELETE']):
                            return True
        
        return False
    
    def _find_inefficient_loop_operations(self, tree: ast.AST, content: str) -> List[str]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.For, ast.While)):
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.Call):
                        if (isinstance(child.func, ast.Attribute) and 
                            child.func.attr in ['append', 'extend', 'insert']):
                            # Проверяем, не используется ли list.append в цикле
                            if 'append' in ast.dump(child):
                                issues.append("list.append() in loop (consider list comprehension)")
                        
                        # Поиск повторных вычислений
                        if (isinstance(child.func, ast.Name) and 
                            child.func.id in ['len', 'max', 'min', 'sum']):
                            issues.append(f"{child.func.id}() called in loop (cache result)")
    ...
    def _has_sync_io_in_loops(self, tree: ast.AST, content: str) -> bool:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.For, ast.While)):
                ...
                io_patterns = ['open(', 'requests.', 'urllib.', 'socket.', 'subprocess.']
...
class HTMLReportGenerator:
    ...
    def generate_dashboard(self, analysis_results: Dict) -> Path:
        ...
        if metrics:
            coupling_scores = analysis_results.get('export_data', {}).get('coupling_scores', {})
            if coupling_scores:
        ...
        stats_data = [
            ...
            ['Проблем производительности', len(analysis_results.get('performance_issues', []))],
        ]
...
class ProjectIntelligence:
    ...
    def analyze_project(self) -> Dict:
        ...
        performance_issues = self.performance_analyzer.find_performance_issues(self.metrics)
        ...
        for func_hash, file_func_pairs in function_hashes.items():
            if len(file_func_pairs) > 1:
                ...
                if len(set(files)) > 1:  # Разные файлы
                    similarity = len(functions) / max(len(self.metrics[f].functions) for f in files)
        ...
        for file_path, metrics in self.metrics.items():
            ...
            for imp in metrics.imports:
                if layer == "models" and "database" in imp:
                    if not any(allowed in file_path for allowed in ["adapter", "interface"]):
                ...
                if "sqlite" in imp.lower() and not self._is_legacy_file(Path(file_path)):
                    self.architecture_violations.append(
    ...
    def _analyze_postgresql_migration(self):
        ...
        migration_progress = len(postgres_files) / (len(postgres_files) + len(sqlite_files)) if (len(postgres_files) + len(sqlite_files)) > 0 else 0
    ...
    def _generate_report(self, security_issues: Optional[List[str]] = None, performance_issues: Optional[List[str]] = None, git_patterns: Optional[Dict] = None) -> Dict:
        ...
        return {
            "summary": {
                ...
                "migration_status": len([v for v in self.architecture_violations if "PostgreSQL" in v]) == 0
            },
            "duplicates_analysis": [{
                "files": d.files,
    ...
    def _generate_ai_recommendations(self) -> List[str]:
        ...
        if len(self.duplicates) > 5:
            recommendations.append("🔄 Рефакторинг: Много дубликатов кода - создайте общие модули")
        
        # Анализируем сложность
        high_complexity = [m for m in self.metrics.values() if m.complexity_score > 100]
        if len(high_complexity) > 10:
            recommendations.append("📉 Упрощение: Много сложных файлов - разбейте на меньшие модули")
        
        # Анализируем legacy
        legacy_ratio = sum(1 for m in self.metrics.values() if m.is_legacy) / len(self.metrics)
        if legacy_ratio > 0.3:
            recommendations.append("🗂️ Миграция: Высокий процент legacy кода - планируйте миграцию")
        
        # Анализируем тесты
        test_ratio = sum(1 for m in self.metrics.values() if m.is_test) / len(self.metrics)
        if test_ratio < 0.2:
            recommendations.append("🧪 Тестирование: Низкое покрытие тестами - добавьте unit тесты")
        
        # Анализируем архитектуру
        if len(self.architecture_violations) > 5:
            recommendations.append("🏗️ Архитектура: Много нарушений - пересмотрите слои приложения")
    ...
    def _is_legacy_file(self, file_path: Path) -> bool:
        ...
        return any(indicator in str(file_path).lower() for indicator in legacy_indicators)
    ...
    if "--analyze" in sys.argv:
        ...
        if results['git_patterns'] and results['git_patterns'].get('bus_factor_risks'):
            ...
            for risk in results['git_patterns']['bus_factor_risks'][:3]:
                print(f"  • {risk['risk_level']}: {risk['file']} (автор: {risk['sole_author']})")
        ...
        if json_file.exists():
            ...
            print("❌ Файл results/project_analysis_enhanced.json не найден. Сначала запустите --analyze")
