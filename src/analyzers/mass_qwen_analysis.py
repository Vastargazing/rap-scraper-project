...
class EmbeddedQwenAnalyzer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.available:
            ...
            logger.info(f"✅ Встроенный Qwen анализатор инициализирован: {self.model_name}")
        else:
    ...
    def analyze_song(self, artist: str, title: str, lyrics: str) -> AnalysisResult:
        ...
        try:
            ...
            logger.error(f"❌ Ошибка анализа встроенного Qwen для {artist} - {title}: {e}")
...
class UnifiedQwenMassAnalyzer:
    ...
    async def load_checkpoint(self) -> bool:
        ...
        try:
            ...
            print(f"📍 Загружен чекпоинт: последняя обработанная запись ID {self.last_processed_id}")
            return True
    ...
    async def get_unanalyzed_records(self, limit: Optional[int] = None, resume: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.db_manager.connection_pool.acquire() as conn:
                ...
                existing = await conn.fetchrow(
                    ...
                    analysis_data['track_id'], analysis_data['analyzer_type']
                )
                
                if existing:
                    print(f"  ⚠️ Анализ уже существует для трека {analysis_data['track_id']}")
                    return True
    ...
    async def run_analysis(self, batch_size: int = 100, max_records: Optional[int] = None, 
                          resume: bool = False, test_mode: bool = False) -> Dict[str, Any]:
        ...
        for i in range(0, len(records), batch_size):
            ...
            print(f"📊 Записи {i+1}-{min(i+batch_size, len(records))} из {len(records)}")
    ...
    async def show_stats_only(self) -> Dict[str, Any]:
        ...
        if db_stats.get('tracks_with_lyrics', 0) > 0:
            coverage = (db_stats.get('qwen_analyzed', 0) / db_stats.get('tracks_with_lyrics', 1)) * 100
        ...
        if self.checkpoint_file.exists():
            try:
                ...
                print(f"\n📍 Найден чекпоинт:")
                print(f"  📄 Последняя обработанная запись: {checkpoint.get('last_processed_id', 0)}")
...
Примеры использования:
  ...
  python src/analyzers/mass_qwen_analysis.py --stats            # Только статистика
    ...
    parser.add_argument('--stats', action='store_true',
                       help='Показать только статистику базы данных')
