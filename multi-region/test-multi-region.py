...
class MultiRegionTester:
        ...
        for region, config in self.regions.items():
            try:
                output = await self.run_kubectl_command(
                    config["context"], "get nodes --no-headers"
                )
                results[region] = bool(output.strip())
                status = "✅" if results[region] else "❌"
                logger.info(
                    f"  {status} {region}: {'Connected' if results[region] else 'Failed'}"
                )
            except Exception as e:
    ...
    async def test_pod_status(self) -> dict[str, dict[str, bool]]:
        ...
        for region, config in self.regions.items():
            ...
            results[region]["postgresql"] = (
                "Running" in pg_output and "1/1" in pg_output
            )
            ...
            results[region]["application"] = (
                "Running" in app_output and "1/1" in app_output
            )
                ...
                if db_secret:
                    ...
                    if config["role"] == "primary":
                        repl_test = await self.run_kubectl_command(
                            config["context"],
                            "exec -n rap-analyzer postgresql-primary-0 -- "
                            "psql -U postgres -c 'SELECT * FROM pg_stat_replication;'",
                        )
                        results[region]["replication_active"] = (
                            "streaming" in repl_test.lower()
                        )
                    else:
                        recovery_test = await self.run_kubectl_command(
                            config["context"],
                            "exec -n rap-analyzer postgresql-replica-0 -- "
        ...
        async with aiohttp.ClientSession(
            ...
            for region, config in self.regions.items():
                ...
                try:
                    ...
                    status = (
                        "✅"
                        if results[region]["service_exists"]
                        and results[region]["endpoints_ready"]
                        else "❌"
                    )
        ...
        for region1, config1 in self.regions.items():
            ...
            for region2, config2 in self.regions.items():
                ...
                try:
                    ...
                    ping_output = await self.run_kubectl_command(
                        config1["context"], ping_cmd
                    )
                    ...
                    if "avg" in ping_output:
                        ...
                        for line in lines:
                            if "avg" in line:
                                avg_latency = (
                                    line.split("/")[-2] if "/" in line else "unknown"
                                )
        ...
        for region, config in self.regions.items():
            ...
            for component in monitoring_components:
                try:
                    ...
                    logger.error(
                        f"Monitoring test failed for {component} in {region}: {e}"
                    )
    ...
    async def generate_test_report(self) -> dict:
            ...
            try:
                ...
                if isinstance(result, dict):
                    if test_name == "cluster_connectivity":
                        ...
                        passed = sum(
                            1
                            for region_result in result.values()
                            for v in region_result.values()
                            if v
                        )
                        total = sum(
                            len(region_result) for region_result in result.values()
                        )
                    else:
                        # Generic counting for other tests
                        passed = sum(
                            1 for v in result.values() if isinstance(v, bool) and v
                        )
        ...
        report["overall_success_rate"] = (
            f"{(total_passed / max(total_tests, 1) * 100):.1f}%"
        )
    ...
    def print_report(self, report: dict):
        ...
        for test_name, summary in report["test_summary"].items():
            ...
            print(
                f"{status} {test_name}: {summary['passed']}/{summary['total']} ({summary['success_rate']})"
            )
...
async def main():
    ...
    try:
        ...
        if exit_code == 0:
            ...
            print(
                "⚠️  Multi-region deployment tests had issues. Check the report above."
            )
