...
class RapTrendAnalyzer:
    ...
    def load_data(self, dataset_path: str = 'data/ml/quick_dataset.pkl') -> pd.DataFrame:
        ...
        try:
            ...
            numeric_columns = ['word_count', 'lyrics_length', 'lines_count', 'avg_words_per_line', 
                             'qwen_confidence', 'qwen_complexity', 'quality_score']
            ...
            df['analysis_date'] = np.random.choice(date_range, size=len(df), replace=True)
    ...
    def analyze_temporal_trends(self, df: pd.DataFrame) -> Dict:
        ...
        sentiment_trends = df.groupby(['quarter', 'qwen_sentiment']).size().unstack(fill_value=0)
        ...
        # === QUALITY TRENDS ===
        quality_trends = df.groupby('quarter')['quality_score'].agg(['mean', 'std']).round(3)
        ...
        logger.info(f"✅ Temporal trends analyzed for {len(sentiment_trends_pct)} periods")
        return trends
    ...
    def cluster_musical_styles(self, df: pd.DataFrame, n_clusters: int = 6) -> Dict:
        ...
        numeric_features = ['word_count', 'lyrics_length', 'lines_count', 
                           'avg_words_per_line', 'qwen_confidence', 'qwen_complexity']
        ...
        for cat_feature in categorical_features:
            if cat_feature in df.columns:
                ...
                feature_data[f'{cat_feature}_encoded'] = df[cat_feature].map(val_to_num).fillna(0)
        ...
        for cluster_id in range(n_clusters):
            ...
            cluster_analysis[cluster_id] = {
                ...
                'top_artists': {str(k): int(v) for k, v in cluster_data['artist'].value_counts().head(5).to_dict().items()},
                'avg_features': {
                ...
                'dominant_sentiment': str(cluster_data['qwen_sentiment'].mode().iloc[0]) if not cluster_data['qwen_sentiment'].mode().empty else 'neutral',
                'dominant_theme': str(cluster_data['theme_category'].mode().iloc[0]) if not cluster_data['theme_category'].mode().empty else 'general',
                'sample_songs': [{str(k): str(v) for k, v in song.items()} for song in cluster_data[['artist', 'title']].head(3).to_dict('records')]
            }
    ...
    def predict_emerging_trends(self, df: pd.DataFrame, forecast_months: int = 6) -> Dict:
        """Predict emerging trends based on growth patterns"""
        logger.info(f"🔮 Predicting emerging trends for next {forecast_months} months...")
        ...
        for month in df['month'].unique():
            ...
            for themes_str in month_data['qwen_themes'].dropna():
                if pd.notna(themes_str) and themes_str.strip():
                    ...
                    themes = [t for t in themes if len(t) > 2 and t not in ['general', 'rap', 'music']]
                    all_themes.extend(themes)
        ...
        for theme in all_themes:
            ...
            if len(monthly_counts) >= 3 and sum(monthly_counts) >= 5:  # Minimum threshold
                # Calculate trend using simple linear regression
                x = np.arange(len(monthly_counts))
                if len(monthly_counts) > 1 and np.std(monthly_counts) > 0:
                    # Simple trend calculation
                    trend_slope = np.polyfit(x, monthly_counts, 1)[0]
                    current_avg = np.mean(monthly_counts[-3:]) if len(monthly_counts) >= 3 else np.mean(monthly_counts)
                    
                    # Predict future popularity
                    predicted_growth = trend_slope * forecast_months
                    growth_rate = (trend_slope / (current_avg + 1)) * 100 if current_avg > 0 else 0
                    ...
                    trend_predictions[theme] = {
                        ...
                        'confidence': min(1.0, sum(monthly_counts) / 50)  # Confidence based on sample size
                    }
        
        # Sort by growth rate and filter
        emerging_trends = {
            theme: data for theme, data in trend_predictions.items()
            if data['growth_rate_percent'] > 10 and data['confidence'] > 0.3
        }
        
        # Sort by growth rate
        emerging_trends = dict(
            sorted(emerging_trends.items(), 
    ...
    def analyze_viral_patterns(self, df: pd.DataFrame) -> Dict:
        ...
        viral_analysis = {
            ...
            'sentiment_distribution': viral_tracks['qwen_sentiment'].value_counts().to_dict(),
            
            # Top viral artists
            'top_viral_artists': viral_tracks['artist'].value_counts().head(10).to_dict(),
            
            # Theme categories
            'theme_category_distribution': viral_tracks['theme_category'].value_counts().to_dict()
        }
        ...
        viral_analysis['comparison'] = {
            'viral_vs_normal': {
                'word_count_ratio': float(viral_tracks['word_count'].mean() / non_viral_tracks['word_count'].mean()),
                'complexity_ratio': float(viral_tracks['qwen_complexity'].mean() / non_viral_tracks['qwen_complexity'].mean()),
                'confidence_ratio': float(viral_tracks['qwen_confidence'].mean() / non_viral_tracks['qwen_confidence'].mean())
            }
        }
    ...
    def generate_trend_report(self, dataset_path: str = 'data/ml/quick_dataset.pkl') -> Dict:
        ...
        try:
            ...
            report = {
                'metadata': {
                    ...
                    'unique_themes': len(self._extract_theme_patterns(df['qwen_themes']))
                },
                'temporal_trends': temporal_trends,
                'style_clusters': style_clusters,
                'emerging_trends': emerging_trends,
                'viral_patterns': viral_patterns,
                'key_insights': self._generate_key_insights(temporal_trends, emerging_trends, viral_patterns)
            }
    ...
    def _generate_key_insights(self, temporal_trends: Dict, emerging_trends: Dict, viral_patterns: Dict) -> List[str]:
        ...
        if emerging_trends.get('emerging_trends'):
            ...
            growth_rate = emerging_trends['emerging_trends'][top_trend]['growth_rate_percent']
            insights.append(f"🔥 Fastest growing theme: '{top_trend}' (+{growth_rate:.1f}% growth rate)")
            ...
            if latest_sentiments:
                ...
                if periods:
                    ...
                    if latest_period in latest_sentiments:
                        dominant_sentiment = max(latest_sentiments[latest_period], 
                                               key=latest_sentiments[latest_period].get)
                        insights.append(f"😊 Current dominant sentiment: {dominant_sentiment}")
        
        # Quality insight
        insights.append(f"💎 {viral_patterns.get('total_viral_tracks', 0)} tracks identified as viral-potential")
    ...
    def create_visualization_dashboard(self, report: Dict) -> str:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Emerging Trends Growth Rate', 'Style Clusters (PCA)', 
                          'Viral Track Characteristics', 'Theme Evolution'),
        ...
        if emerging:
            ...
            fig.add_trace(
                go.Bar(x=growth_rates, y=themes, orientation='h', name='Growth Rate %',
        ...
        if 'pca_visualization' in clusters:
            ...
            for cluster_id in unique_clusters:
                cluster_indices = [i for i, c in enumerate(pca_data['clusters']) if c == cluster_id]
        ...
        if viral:
            ...
            fig.add_trace(
                go.Bar(x=characteristics, y=values, name='Viral Features',
        ...
        if temporal:
            ...
            if latest_period and latest_period in temporal:
                ...
                fig.add_trace(
                    go.Bar(x=themes, y=counts, name='Theme Frequency',
    ...
    try:
        ...
        if emerging:
            ...
            for i, (theme, data) in enumerate(list(emerging.items())[:10], 1):
                ...
                print(f"  {i:2d}. {theme:20} | Growth: +{growth:5.1f}% | Confidence: {confidence:.2f}")
        ...
        if viral:
            ...
            print(f"  Average word count: {viral.get('avg_characteristics', {}).get('word_count', 0):.0f}")
            print(f"  Top viral theme: {list(viral.get('viral_themes', {}).keys())[0] if viral.get('viral_themes') else 'N/A'}")
        
        print(f"\n📊 ANALYSIS COMPLETE!")
        print(f"Report saved to: ./models/trend_analysis_report.json")
