...
Генератор подробной статистики по результатам AI-анализов

НАЗНАЧЕНИЕ:
- Сбор и агрегация статистики по AI-анализу корпуса треков
- Жанровый, эмоциональный и качественный анализ
- Экспорт JSON-отчетов для дальнейшей визуализации
...
РЕЗУЛЬТАТ:
- JSON-отчеты в results/ и/или вывод в консоль
- Сводные CSV/JSON для дашбордов
...
class AIAnalysisStatsGenerator:
    ...
    def _get_quality_stats(self, conn) -> dict[str, Any]:
        ...
        return {
            "quality_averages": {
                "authenticity": round(quality_averages[0], 3)
                if quality_averages[0]
                else None,
                "creativity": round(quality_averages[1], 3)
                if quality_averages[1]
                else None,
                "commercial_appeal": round(quality_averages[2], 3)
                if quality_averages[2]
                else None,
                "uniqueness": round(quality_averages[3], 3)
                if quality_averages[3]
                else None,
                "ai_likelihood": round(quality_averages[4], 3)
                if quality_averages[4]
                else None,
            },
    ...
    def _get_artist_stats(self, conn) -> dict[str, Any]:
        ...
        return {
            ...
            "top_quality_artists": [
                (artist, round(auth, 3), round(creat, 3), count)
    ...
    def _get_commercial_stats(self, conn) -> dict[str, Any]:
        ...
        cursor.execute("""
            ...
            LIMIT 20
        """)
        high_commercial_tracks = [
            (artist, title, round(appeal, 3))
            for artist, title, appeal in cursor.fetchall()
        ]
        ...
        """)
        commercial_by_genre = [
            (genre, round(avg, 3)) for genre, avg in cursor.fetchall()
        ]
    ...
    def _print_summary(self, stats: dict[str, Any]):
        ...
        print(
            f"📊 Покрытие: {overview['analyzed_songs']:,} анализов из {overview['total_songs']:,} треков ({overview['coverage_percent']}%)"
        )
        print(
            f"🎵 Осталось проанализировать: {overview['remaining_to_analyze']:,} треков"
        )
