...
class PytestBenchmarkIntegration:
    ...
    def generate_benchmark_test(self, analyzer_type: str, test_texts: List[str], output_file: str):
        """Генерация pytest benchmark тестов"""
        if not self.available:
            self.monitor.display.print_warning("pytest не доступен для benchmark генерации")
            return
...
'''
        ...
        try:
            ...
            self.monitor.display.print_success(f"Benchmark тесты сгенерированы: {output_file}")
            self.monitor.display.print_progress_info("Запустите: pytest " + output_file + " --benchmark-only")
            
        except Exception as e:
            self.monitor.display.print_error(f"Ошибка генерации benchmark тестов: {e}")
    
    def run_benchmark_tests(self, test_file: str, json_output: Optional[str] = None) -> Optional[Dict]:
        ...
        try:
            ...
            cmd = ["python", "-m", "pytest", test_file, "--benchmark-only", "--benchmark-sort=mean"]
            
            if json_output:
                cmd.extend(["--benchmark-json", json_output])
            
            self.monitor.display.print_progress_info(f"Запуск benchmark тестов: {test_file}")
            ...
            if result.returncode == 0:
                ...
                if json_output:
                    try:
                        ...
                        self.monitor.display.print_warning(f"Не удалось прочитать JSON результаты: {e}")
                
                return {"status": "success", "output": result.stdout}
            else:
                self.monitor.display.print_error(f"Benchmark тесты завершились с ошибкой: {result.stderr}")
                return None
    ...
    def compare_benchmark_results(self, results1: Dict, results2: Dict, analyzer1: str, analyzer2: str):
        ...
        table = Table(title=f"📊 Benchmark Comparison: {analyzer1} vs {analyzer2}", box=box.ROUNDED)
...
class RichDisplayManager:
    ...
    def print_analyzer_info(self, analyzer_type: str, info: dict):
        ...
        if self.use_rich and self.console:
            table = Table(title=f"📊 Analyzer: {analyzer_type}", show_header=False, box=box.SIMPLE)
            table.add_column("Property", style="cyan", width=20)
            table.add_column("Value", style="green")
            
            table.add_row("Type", analyzer_type)
            table.add_row("Available", "✅ Yes" if info.get('available', True) else "❌ No")
            table.add_row("Features", ", ".join(info.get('supported_features', [])))
            
            self.console.print(table)
        else:
            print(f"📊 Analyzer: {analyzer_type}")
            print(f"   Available: {'✅ Yes' if info.get('available', True) else '❌ No'}")
    ...
    def print_metrics_table(self, metrics: EnhancedMetrics):
        ...
        if self.use_rich and self.console:
            table = Table(title=f"📈 Performance Metrics: {metrics.analyzer_name}", box=box.ROUNDED)
            ...
            table.add_row("⚡ CPU Efficiency", f"{metrics.cpu_efficiency:.2f}", "items/cpu%")
    ...
    def print_load_test_results(self, results: Dict[str, Any]):
        ...
        if self.use_rich and self.console:
            table = Table(title=f"🔥 Load Test Results: {results['analyzer_type']}", box=box.HEAVY)
            table.add_column("Metric", style="red bold", width=20)
            table.add_column("Value", style="bright_yellow", width=15)
            table.add_column("Unit", style="dim", width=10)
            
            table.add_row("🚀 Requests/sec", f"{results['requests_per_second']:.1f}", "req/s")
            table.add_row("✅ Success Rate", f"{100 - results['error_rate_percent']:.1f}", "%")
            table.add_row("❌ Error Rate", f"{results['error_rate_percent']:.1f}", "%")
            table.add_row("⏱️ Avg Response", f"{results['avg_response_time']:.3f}", "seconds")
            table.add_row("👥 Concurrent Users", f"{results['concurrent_users']}", "users")
    ...
    def print_progress_info(self, message: str, current: Optional[int] = None, total: Optional[int] = None):
    ...
    def print_analyzer_comparison(self, comparison_results: Dict[str, Dict]):
        ...
        if self.use_rich and self.console:
            ...
            sorted_results = sorted(
                ...
                key=lambda x: x[1].get('items_per_second', 0) if not x[1].get('error') else 0,
                reverse=True
            )
    ...
    async def benchmark_with_profiling(
        ...
        for i, text in enumerate(test_texts):
            ...
            try:
                ...
                error_count += 1
                text_time = time.time() - text_start_time
                self.logger.warning(f"⏰ Timeout processing text {i+1} after {timeout_per_text}s")
        ...
        async def worker(worker_id: int):
            ...
            while time.time() - start_time < duration_seconds:
                ...
                try:
                    ...
                    import inspect
                    if inspect.iscoroutinefunction(analyzer.analyze_song):
                        result = await analyzer.analyze_song("Unknown", f"Worker_{worker_id}", text)
                    else:
...
async def main():
    ...
    parser.add_argument("--mode", choices=["benchmark", "profile", "compare", "load", "pyspy", "pytest"], 
    ...
    parser.add_argument("--timeout", type=float, default=30.0, help="Timeout per text analysis (seconds)")
    ...
    try:
        if args.mode == "benchmark":
            ...
            monitor.display.print_progress_info(f"Starting benchmark for {analyzer_type}")
            
            metrics = await monitor.benchmark_with_profiling(
                analyzer_type, test_texts, enable_profiling=True, timeout_per_text=args.timeout
            )
            ...
            monitor.display.print_progress_info(f"Starting py-spy profiling for {analyzer_type}")
            
            profile_file = await monitor.py_spy_analysis(analyzer_type, test_texts, args.duration)
            if profile_file:
                monitor.display.print_success(f"py-spy profile saved to: {profile_file}")
            else:
                monitor.display.print_warning("py-spy profiling failed or not available")
            
        elif args.mode == "load":
            analyzer_type = args.analyzer or "advanced_algorithmic"
            
            monitor.display.print_progress_info(f"Starting load test for {analyzer_type}")
            ...
            if args.output:
                ...
                monitor.display.print_success(f"Load test results saved to: {args.output}")
            
        elif args.mode == "compare":
            analyzers = ["advanced_algorithmic"] if not args.all else ["advanced_algorithmic", "qwen", "emotion_analyzer"]
            
            monitor.display.print_progress_info(f"Starting comparison of {len(analyzers)} analyzers")
            ...
            for analyzer_type in analyzers:
                try:
                    ...
                    monitor.display.print_warning(f"Failed to benchmark {analyzer_type}: {e}")
            ...
            if hyperfine_results:
                ...
                monitor.display.print_warning("Hyperfine comparison failed or not available")
                monitor.display.print_success("✨ Internal comparison completed! Install hyperfine for external benchmarks:")
            ...
            if args.output:
                ...
                monitor.display.print_success(f"Comparison results saved to: {args.output}")
        
        elif args.mode == "pytest":
            analyzer_type = args.analyzer or "advanced_algorithmic"
            
            monitor.display.print_progress_info(f"Generating pytest benchmark tests for {analyzer_type}")
            
            # Генерируем benchmark тесты
            test_file = f"test_benchmark_{analyzer_type}.py"
            monitor.pytest_benchmark.generate_benchmark_test(analyzer_type, test_texts, test_file)
            
            # Опционально запускаем тесты
            if args.output:
                json_output = args.output.replace('.json', '_benchmark.json')
                results = monitor.pytest_benchmark.run_benchmark_tests(test_file, json_output)
                
                if results:
                    monitor.display.print_success(f"Pytest benchmark результаты сохранены в: {json_output}")
