...
class PrometheusMetrics:
    ...
    def __init__(self, port=8090):
        # Счетчики
        self.songs_processed = Counter(
            "scraper_songs_processed_total", "Total processed songs"
        )
        self.songs_added = Counter("scraper_songs_added_total", "Total added songs")
        self.songs_skipped = Counter(
            "scraper_songs_skipped_total", "Total skipped songs", ["reason"]
        )
        self.api_calls = Counter(
            "scraper_api_calls_total", "Total API calls", ["endpoint"]
        )
        self.errors = Counter("scraper_errors_total", "Total errors", ["type"])

        # Гистограммы времени
        self.processing_time = Histogram(
            "scraper_processing_duration_seconds", "Song processing time"
        )
        self.batch_save_time = Histogram(
            "scraper_batch_save_duration_seconds", "Batch save time"
        )
        self.api_response_time = Histogram(
            "scraper_api_response_duration_seconds", "API response time"
        )

        # Gauge для текущих значений
        self.memory_usage = Gauge(
            "scraper_memory_usage_mb", "Current memory usage in MB"
        )
        ...
        self.circuit_breaker_state = Gauge(
            "scraper_circuit_breaker_open", "Circuit breaker state (1=open, 0=closed)"
        )
...
class AsyncPostgreSQLManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"🏊 PostgreSQL pool создан: {self.min_size}-{self.max_size} соединений"
            )
        except Exception as e:
    ...
    async def song_exists_batch(self, urls: list[str]) -> set:
        ...
        async with self.get_connection() as conn:
            result = await conn.fetch(
                "SELECT url FROM tracks WHERE url = ANY($1)", urls
            )
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if any(
                        ...
                        delay = base_delay * (backoff_factor**attempt)
                        logger.warning(
                            f"❓ Неизвестная ошибка, пауза {delay:.1f}с: {e}"
                        )
...
class PriorityBatchProcessor:
    ...
    def _should_flush(self) -> bool:
        ...
        return (
            ...
            or queue_size > self.max_queue_size * 0.8
        )
...
class UltraOptimizedScraper(OptimizedPostgreSQLScraper):
    ...
    async def enhanced_get_artist_songs(self, artist_name: str, max_songs: int = 500):
        ...
        if cached_songs:
            logger.info(
                f"📋 Найдено в Redis кэше: {len(cached_songs)} песен для {artist_name}"
            )
        ...
        try:
            artist = await asyncio.get_event_loop().run_in_executor(
                ...
                lambda: self.genius.search_artist(
                    artist_name, max_songs=min(max_songs, 100), sort="popularity"
                ),
            )
    ...
    async def ultra_process_song(
        self, song, song_number: int, artist_name: str
    ) -> ScrapingStatus:
        ...
        song_hash = self.generate_song_hash(
            artist_name, song.title, getattr(song, "lyrics", "")[:200]
        )
