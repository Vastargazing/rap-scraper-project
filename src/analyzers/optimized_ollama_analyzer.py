...
class OptimizedOllamaAnalyzer(BaseAnalyzer):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        if self.available:
            logger.info(
                f"✅ Оптимизированный Ollama анализатор готов: {self.model_name}"
            )
            logger.info(
                f"🔧 Режим экономии: {self.economy_mode}, макс CPU: {self.max_cpu_usage}%"
            )
        else:
    ...
    def _initialize_best_model(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"💻 Система: RAM {system_info['ram_gb']:.1f}GB, CPU {system_info['cpu_percent']:.1f}%"
            )
    ...
    def _select_optimal_model(
        self, system_info: dict, installed_models: list[str]
    ) -> str | None:
        ...
        logger.info(
            f"🔍 Поиск оптимальной модели для {ram_gb:.1f}GB RAM, CPU {cpu_percent:.1f}%"
        )
    ...
    def _adjust_settings_for_model(self, model_name: str, system_info: dict):
        ...
        logger.info(
            f"⚙️ Настройки для {model_name}: контекст {self.context_window}, токены {self.max_tokens}"
        )
    ...
    def _test_model_performance(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200 and test_time < 10:  # Должно быть быстро
                logger.info(
                    f"✅ Тест модели {self.model_name} прошел успешно ({test_time:.1f}с)"
                )
                return True
            logger.warning(
                f"⚠️ Модель {self.model_name} работает медленно ({test_time:.1f}с)"
            )
            return False
    ...
    def _install_lightweight_model(self) -> bool:
        ...
        try:
            logger.info(
                f"📥 Устанавливаем легкую модель {target_model} (это займет несколько минут)..."
            )
    ...
    def analyze_song(self, artist: str, title: str, lyrics: str) -> AnalysisResult:
        ...
        if self.resource_monitor.is_system_overloaded(self.max_cpu_usage):
            logger.warning("⚠️ Система перегружена, используем экономичный режим")
        ...
        try:
            ...
            logger.error(
                f"❌ Оптимизированный анализ не удался для {artist} - {title}: {e}"
            )
