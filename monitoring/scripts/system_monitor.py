...
class SystemMonitor:
    ...
    def get_cpu_stats(self) -> dict[str, Any]:
        ...
        if hasattr(os, "getloadavg"):
            try:
                cpu_stats["load_average"] = os.getloadavg()
            except:
    ...
    def get_process_stats(self) -> dict[str, Any]:
        ...
        stats = {"total_processes": len(psutil.pids()), "python_processes": []}
    ...
    def monitor_loop(self, interval: int = 60):
        ...
        while True:
            try:
                ...
                print(
                    f"💾 Memory Usage: {metrics['memory']['usage_percent']:.1f}% ({metrics['memory']['used_gb']:.1f}GB/{metrics['memory']['total_gb']:.1f}GB)"
                )
                print(
                    f"💿 Disk Usage: {metrics['disk']['usage_percent']:.1f}% ({metrics['disk']['used_gb']:.1f}GB/{metrics['disk']['total_gb']:.1f}GB)"
                )
                print(
                    f"🐍 Python Processes: {len(metrics['processes']['python_processes'])}"
                )

                # Save metrics
                metrics_file = os.path.join(
                    os.path.dirname(__file__), "..", "metrics", "system_metrics.json"
                )
